...
let width = 1024 and height = 1024

let line_height = 25

let padding_left = 50 and padding_right = 50 and padding_up = 50 and padding_down = 50
...
let draw_ball ?select:(select=false) ball =
  let p = Rules.position_of_ball ball
  and size = !cell_size in
  let x = padding_left + Position.proj_x p * size + (size / 2)
  and y = padding_left + Position.proj_y p * size + (size / 2) in
  ...
  begin
    ...
    if !colors_generated then
      begin
          let f cb = Rules.eq_ball (snd cb) ball in (* color stability is managed here *)
          let color = 
          if List.exists f !colors then
              fst (List.find f !colors)
          else (generate_new_color G.white) in
                G.set_color color;
      end
    else
  ...
  (* drawing pokeball layer *)
  (* first draw black outlines *)
  G.set_color G.black;
  G.draw_circle x y (radius / 5);
  G.draw_circle x y (radius / 4);
  G.draw_circle x y (radius - 1);
  (* horizontal middle bar *)
  let radiusInner = radius - radius / 4 in
  G.fill_rect (x - radius) (y + 1) radiusInner 3;
  G.fill_rect (x + radius / 4) (y + 1) radiusInner 3;
  (* drawing bottom white half circle *)
  G.set_color G.white;
  for i = (radius / 4 + 1) to (radius - 2) do
      G.draw_arc x y i i 180 360
  done; ()
